...
class journal_print(report_sxw.rml_parse):
    ...
    def lines(self, period_id, journal_id, sort_selection='date', form={}, *args):
        ids_final = []
        if form['state']=='bydate':
            for journal in journal_id:
                self.cr.execute("select id from account_move_line where journal_id=%s and state<>\'draft\' " \
                        "and date >= %s and date <= %s order by ("+ sort_selection +"),id", (journal, form['date1'], form['date2']))
            ...
            for line_id in ids_final:
                ...
                line_ids.append(a)
            return line_ids
        elif type(period_id)==type([]) and form['state'] != 'bydate':
            for journal in journal_id:
    ...
    def _sum_credit(self, period_id, journal_id):
        ...
        return self.cr.fetchone()[0] or 0.0
report_sxw.report_sxw('report.account.journal.period.print.bias', 'account.journal.period', 'addons/bias_account/report/wizard_account_journal.rml', parser=journal_print,header=False)
