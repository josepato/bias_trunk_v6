...
--RAISE NOTICE 'Prueba : %', array_dims(all_partners);
    ...
    -- JOURNAL
    IF journal_ids <> '' THEN
	query_journal := ' AND (l.journal_id IN '||journal_ids||') ' ;
    END IF;
...
RAISE NOTICE 'partner_type : %', partner_type ;
	...
	CREATE INDEX all_lines_date_maturity_index
	ON all_lines
	USING btree (date_maturity);

	CREATE INDEX all_lines_currency_id_index
	ON all_lines
	USING btree (currency_id);

	CREATE INDEX all_lines_amount_currency_index
	ON all_lines
	USING btree (amount_currency);

	CREATE INDEX all_lines_reconcile_id_index
	ON all_lines
	USING btree (reconcile_id);

	CREATE INDEX all_lines_cost_center_id_index
	ON all_lines
	USING btree (cost_center_id);
	...
	*/
	...
	LOOP
		...
		AND pay.id = replace(prop.value, 'account.payment.term,', '')::integer
