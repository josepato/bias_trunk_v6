...
fiscal_fields = {
    ...
    'fiscal_statements_id': {'string': 'Fiscal Statement', 'type': 'many2one', 'relation': 'fiscal.statements', 'readonly': True, 'default': lambda *a:2},
    'fiscalyear': {
...
def _get_period(self, cr, uid, data, context={}):
    ...
    statement = pool.get('financial.reports').browse(cr, uid, report_id).statement_income_id.id
    period_id = False
    if len(ids):
        period_id = ids[0]
    return {
...
class wizard_fiscal_statements_income(wizard.interface):
...
wizard_fiscal_statements_income('fiscal.statements.income')
