...
def getSello(cadena, privkeyfname, passphrase):
    ...
    if res:
        raise osv.except_osv(('Error !'), ('Error al borrar archivo tempora!.'))
        return ""
...
def getLineaReporte(obj, cancelada=False):
    linea = LineaReporte()
    cfdroot = obj.getroot()
    for item in cfdroot.iterchildren():
        if testItem(item, "Emisor"):
    ...
    linea.agregaElem(cfdroot, "subTotal")
    linea.agregaElem(impuestos, "totalImpuestosTrasladados")
