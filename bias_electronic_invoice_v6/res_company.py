...
class res_company(osv.osv):
	...
	_columns = {
		...
		'tralix_customer_key': fields.char('Tralix Customer Key', size=40, invisible=True, required=False, ),
		'tralix_host': fields.selection([
			('pruebastfd.tralix.com', 'Pruebas Tralix'),
			('timbrador.tralix.com', 'Tralix Produccion'),
			], 'Talix Host',required=False, help="""Seleccionar el Host de Tralix que desea utilizar."""),
	...
	def getTralixVariables(self, cr, uid, company_id):
		res = self.read(cr, uid,[company_id,], ['tralix_customer_key','tralix_host'])
		return res[0]['tralix_customer_key'], res[0]['tralix_host']
