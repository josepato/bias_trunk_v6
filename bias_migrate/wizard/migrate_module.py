...
#        print 'xmlrpc_port=',config['xmlrpc_port']
...
class change_url_from(osv.osv_memory):
    _name = "change.url.from"
    _description = "Change URL FROM"

    def _server_from(self, cr, uid, context=None):
        return self.pool.get('migrate.module')._get_defaults(cr, uid, 'server_from', context=context)

    def _port_from(self, cr, uid, context=None):
        return self.pool.get('migrate.module')._get_defaults(cr, uid, 'port_from', context=context)
    ...
    def save(self, cr, uid, ids, context=None):
        ...
        mod_obj.write(cr, uid, data['ids'], {'server_from':form['server_from'], 'port_from':form['port_from'], 'url_from':url, 'db_from':[(6, 0, db_ids)]})
        return {
            'view_type': 'form',
            'view_mode': 'form,tree',
...
#        'migrate_id': fields.many2one('migrate.module', 'Migrate', select=True),
    }
...
class migrate_module(osv.osv_memory):
    ...
    def _get_db(self, cr, uid, context=None):
    ...
    def _get_defaults(self, cr, uid, field_name, context=None):
        ...
        data['form'] = form
        db_ids = []
        if field_name == 'db_from':
            form['db_from'] = self.pool.get('migrate.migrate').list_db(cr, uid, data)
#            for db in self.pool.get('migrate.migrate').list_db(cr, uid, data):
...
#            print 'form=',data['form']
    ...
    _columns = {
        ...
        'db_from': fields.selection(_get_db, 'Data Bases', method=True, size=32),
#        'db_from': fields.many2many('migrate.db', 'migrate_db_rel', 'migrate_id', 'db_id', 'Data Bases'),
    }
    _defaults = {
        'count': 1000,
        'server_from': 'localhost',
        'server_to': 'localhost',
        'port_from': '8069',
        'port_to': '8069',
        'url_from': _url_from,
