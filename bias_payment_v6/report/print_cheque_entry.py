...
#[[ texto(int(o.amount)) ]] [[ o.mode.journal.currency.name ]] [[ ('%.2f' % o.amount)[-2:] ]]/100 [[ o.mode.journal.currency.code ]]
	def _text(self, obj):
		currency_name = {'MN':'PESOS','USD':'DOLLAR','EUR':'EUROS'}
		amount = text.text(int(obj.amount))+' '
		amount = amount + currency_name[obj.mode.journal.currency.name]+' '
		amount = amount + ('%.2f' % obj.amount)[-2:] +'/100 '+obj.mode.journal.currency.name
		return amount

	def _get_name(self, obj, line):
		name = line.name
		if line.invoice:
			name = 'FACTURA '+line.ref
		return name
...
report_sxw.report_sxw('report.payment.cheque.entry', 'payment.cheque',
	'addons/bias_payment_v6/report/print_cheque_entry.rml', parser=print_cheque, header=False)
