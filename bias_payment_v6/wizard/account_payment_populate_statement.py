    ...
    def populate_statement(self, cr, uid, ids, context=None):
        ...
        for line in line_obj.browse(cr, uid, line_ids, context=context):
            ...
            if line.move_line_id:
                ...
                result = voucher_obj.onchange_partner_id(cr, uid, [], partner_id=line.partner_id.id, journal_id=statement.journal_id.id, \
                        price=abs(amount), currency_id= statement.currency.id, ttype='payment', date=line.date or time.strftime('%Y-%m-%d'),context=context)
                voucher_res = {
                        'type': 'payment',
                        'name': line.name,
            ...
            elif line.partner_id.property_account_payment:
                st_line_id = statement_line_obj.create(cr, uid, {
                    'name': line.order_id.reference or '?',
                    'amount': - amount,
                    'type': 'supplier',
                    'partner_id': line.partner_id.id,
                    'account_id': line.partner_id.property_account_payment.id,
                    'statement_id': statement.id,
                    'ref': line.communication,
                    }, context=context)
