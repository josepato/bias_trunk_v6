...
class email_template_send_wizard(osv.osv_memory):
    ...
    _defaults = {
        ...
        'full_success': lambda self,cr,uid,ctx: False,
#        'full_success': False,
        'attachment_ids': lambda self,cr,uid,ctx: []
#        'attachment_ids': [], 
    }
    ...
    def save_to_mailbox(self, cr, uid, ids, context=None):
        ...
        for id in context['src_rec_ids']:
            ...
            screen_vals = type(screen_vals).__name__ == 'dict' and screen_vals or  screen_vals[0]
            ...
            ctx = [('res_model','=',context['src_model']),('res_id','=',context['active_id'])]
            for attachment_id in att_obj.search(cr, uid, ctx):
                if template.ext:
                    if template.ext == 'all':
                        attachment_ids += att_obj.search(cr, uid, ctx)
                    else:
                        for ext in template.ext.split(','):
                            if ext.strip().lower() == att_obj.browse(cr, uid, attachment_id).datas_fname[-3:].lower():
                                attachment_ids.append(attachment_id)
